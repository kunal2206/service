...
class OngoingOrderDetailsView extends GetView<OngoingOrderDetailsController> {
  const OngoingOrderDetailsView({Key? key}) : super(key: key);

  void eventHandler(Map<String, dynamic> data) {
    controller.findOrderById(data['orderId']).then((order) {
      controller.ongoingOrder.value = order;
    });
  }
                        ...
                        child: Container(
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              mainAxisSize: MainAxisSize.max,
                              children: [
                                Obx(() => controller.ongoingOrder.value != null
                                    ? Column(
                                        children: [
                                          orderIdSection(controller
                                              .ongoingOrder.value!.id
                                              .substring(15)),
                                          dateAndTimeSection(controller
                                            ...
                                            child: Row(
                                              ...
                                              mainAxisAlignment:
                                                  MainAxisAlignment
                                                      .spaceBetween,
                                              children: [
                                                servicePersonSection(controller
                                                    .ongoingOrder
                                                    .value!
                                                    .serviceMan),
                                                orderStatusSection(
                                                    statusToString(codeToStatus(
                                                        controller.ongoingOrder
                                                            .value!.status))),
                                              ],
                                            ),
                                          ),
                                          OngoingDynamicContainer(
                                            controller: controller,
                                            status: controller
                                                .ongoingOrder.value!.status,
                                          ),
                                        ],
                                      )
                                    : SizedBox(
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                        ...
                        children: controller.ongoingOrder.value!.orderItems !=
                                null
                            ? controller.ongoingOrder.value!.orderItems!
                                .map((orderItem) {
                                return Row(
                                  children: [
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.6,
                                      child: FutureBuilder(
                                          future: controller.findServiceById(
                                              orderItem.productId),
                                          builder: (context,
